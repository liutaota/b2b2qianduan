<template>
  <el-dialog :visible.sync="dialogFormVisible" :close-on-click-modal="false" :before-close="cancel" destroy-on-close top="4vh" width="80%">
    <div class="search_wrap" @keyup.enter="query">
      <el-input v-model.trim="form.customsetid" :placeholder="placeholder.id" style="width: 200px;margin-right:10px" />
      <el-input v-model.trim="form.customsetname" :placeholder="placeholder.name" style="width: 200px;margin-right:10px" />
      <!-- <el-cascader
...
export default {
  ...
  watch: {
    form: {
      handler(val) {
        if (val.customsetid === '' && val.customsetname === '') {
          this.query();
        }
      },
      deep: true
      ...
      this.$http.get(this.aggregate.path, { params: { ...form }}).then(res => {
        ...
        this.goodsChoice.forEach((data, i) => {
          res.records.forEach((item, index) => {
            if (item.customsetid === data.customsetid) {
