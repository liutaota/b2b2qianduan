<template>
  <el-dialog :visible.sync="dialogFormVisible" :close-on-click-modal="false" :before-close="cancel" destroy-on-close top="4vh" width="80%">
    ...
    <div class="transfer">
      ...
      <div>
        <Tabel :columns="columns" :fixed="false" :table-data="goodsChoice" :is_height="400" :page-index="form" @handleSelectionChange="handleSelectionChange">
          <el-table-column
            slot="index"
            fixed
            type="selection"
...
export default {
  ...
  watch: {
    goodsList(val) {
      val.forEach(item => {
        const { goodsId: goodsid, goodsName: goodsname, factoryname, currencyname, goodstype } = item;
        this.goodsChoice.push({ goodsid, goodsname, factoryname, currencyname, goodstype });
      });
    },
    form: {
      handler(val) {
