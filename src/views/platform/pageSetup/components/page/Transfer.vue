<template>
  <el-dialog :visible.sync="dialogFormVisible" :close-on-click-modal="false" :before-close="cancel" destroy-on-close top="4vh" width="80%">
    <div class="search_wrap" @keyup.enter="query">
      <el-input v-model.trim="form.goodsid" :placeholder="placeholder.id" style="width: 200px;margin-right:10px" />
      <el-input v-model.trim="form.goodsname" :placeholder="placeholder.name" style="width: 200px;margin-right:10px" />
      <el-cascader
...
export default {
  ...
  mounted() {
    ...
    this.query();
    if (this.aggregate.listKey.key === 'goodsid') {
    ...
    } else if (this.aggregate.listKey.key === 'goodssetid') {
      this.columns = [
        {
          prop: 'goodssetid',
          label: '货品Id',
  ...
  methods: {
    ...
    query() {
      ...
      if (this.aggregate.listKey.key === 'customsetid') {
        form.customsetid = this.form.goodsid;
        form.customsetname = this.form.goodsname;
        // delete form.goodsid;
        // delete form.goodsname;
      }
      if (this.aggregate.listKey.key === 'goodssetid') {
        form.goodssetid = this.form.goodsid;
        form.goodssetname = this.form.goodsname;
        // delete form.goodsid;
        // delete form.goodsname;
      }
