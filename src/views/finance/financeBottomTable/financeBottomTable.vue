<template>
  <div style="margin: 0 10px">
    <Screen v-permission="'bottomTableScreen'" @query="query" />
    <div>
      <el-button
        type="primary"
        v-permission="'bottomTableExcel'"
        @click="exportExcel"
        >Excel导出</el-button
      >
    </div>
        ...
        <el-table-column slot="handleColumn" label="操作" fixed="right">
          <template slot-scope="scope">
            <el-button
              size="mini"
              type="primary"
              v-permission="'bottomTableOrderSee'"
              @click="
                orderForm = { ...scope.row };
                orderSeeMask = true;
              "
...
export default {
  ...
  methods: {
    ...
    query(queryForm) {
      ...
      this.$http
        .get("stateMent/getFinancialStatementList", {
          params: { ...this.queryForm }
        })
        .then(res => {
          this.financepData = res.records.map((item, index) => {
            item.id_order = item.id; // id有重复，进行重新赋值，防止表格循环key值重复
            item.id = index;
            item.platformAmount = this.$math.subtract(item.orderAmount, item.goodsAmount)
    ...
    //excel导出
    exportExcel() {
      let loadingInstance = load.service({ fullscreen: true });
      this.$http
        .get("excel/getFinancialStatementListExcel", { params: this.queryForm })
        .then(res => {
